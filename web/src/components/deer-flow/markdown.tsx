  ...
  const components: ReactMarkdownOptions["components"] = useMemo(() => {
    return {
      ...
      code({ node, inline, className, children, ...rest }: any) {
        // 检查是否为 echarts 代码块
        if (!inline && (className === "language-echarts" || (node as any)?.lang === "echarts")) {
          try {
            const option = JSON.parse(children as string);
            return <EChartsView option={option} />;
          } catch (e) {
            return <pre style={{ color: "red" }}>ECharts 配置解析失败</pre>;
          }
        }
        return (
