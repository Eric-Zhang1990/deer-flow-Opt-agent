...
function MessageListItem({
  ...
  if (message) {
    if (
      ...
      if (message.agent === "planner" || message.agent === "curve" || message.agent === "power" || message.agent === "temp") {
      ...
      {shouldShowPlan && (
        ...
        >
          <Card className="w-full">
            ...
            <CardContent>
              {/* 直接展示message.content中的文本和图片，支持混排 */}
              {(() => {
                ...
                try {
                  ...
                  if (Array.isArray(parsed)) {
                    ...
                    contentArr = [parsed];
                  } else if (parsed && typeof parsed === "object" && parsed.thought) {
                    // 兼容旧格式
                    if (Array.isArray(parsed.thought)) {
                      contentArr = parsed.thought;
                ...
                return contentArr.map((item, idx) =>
                  typeof item === "string" && /^https?:\/\/.+\.(png|jpe?g|gif|webp)$/i.exec(item) ? (
                    <img
                      key={idx}
                      src={item}
                      alt="plan-img"
                    ...
                    <Markdown key={idx} className="opacity-80" animated={message.isStreaming}>
                      {item}
                    </Markdown>
                  )
                );
