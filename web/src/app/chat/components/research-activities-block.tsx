...
function ActivityListItem({ messageId }: { messageId: string }) {
  const message = useMessage(messageId);
  if (message) {
    // 对于VPP消息，即使没有toolCalls也要显示
    if (message.agent === "vpp" && !message.isStreaming) {
      return null; // 让ActivityMessage来处理VPP消息的显示
    }
    ...
    if (!message.isStreaming && message.toolCalls?.length) {
      for (const toolCall of message.toolCalls) {
        ...
        // } else if (toolCall.name === "my_custom_tool") {
        //   return <MyToolCall key={toolCall.id} toolCall={toolCall} />;
        } else {
