...
# import asyncio
#
# async def stream_agent(user_input, debug, max_plan_iterations, max_step_num, enable_background_investigation):
#     async for s in run_agent_workflow_async(
...
#                 final_state = s.get(list(s.keys())[0], {})
#                 if final_state.get("file_path"):
#                     yield f"生成的房价趋势图保存在：{final_state.get('file_path')}\n"
#             yield f"Output: {s}\n"
#         except Exception as e:
#             yield f"生成房价趋势图失败: {str(e)}\n"
#
#
# interface = gr.Interface(
#     fn=stream_agent,
#     inputs=[
#         gr.Textbox(label="用户输入", value="上海"),
#         gr.Checkbox(label="调试模式", value=False),
#         gr.Slider(minimum=1, maximum=10, step=1, label="最大计划迭代次数", value=1),
#         gr.Slider(minimum=1, maximum=10, step=1, label="最大步骤数", value=3),
#         gr.Checkbox(label="启用背景调查", value=True),
#     ],
#     outputs=gr.Textbox(label="流式输出", lines=10),
#     title="Agent 工作流流式输出",
#     description="输入城市名称，例如：上海、北京，查看房价趋势图。",
